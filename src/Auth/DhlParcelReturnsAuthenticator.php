...
class DhlParcelReturnsAuthenticator implements Authenticator
{
    ...
    public function set(PendingRequest $pendingRequest): void
    {
        ...
        if (! $this->apiKey && ! ($this->username && $this->password)) {
            throw new InvalidArgumentException(
                'DHL Parcel Returns authentication requires either an API key, username/password credentials, or OAuth2 credentials (api_key, client_secret, username, password).'
            );
        }
    ...
    private function getAccessToken(): string
    {
        ...
        if (! isset($payload['access_token']) || ! is_string($payload['access_token'])) {
            throw new DhlParcelReturnsApiException(
                message: 'DHL Parcel Returns OAuth2 token response is missing a valid access_token.',
